...
Class Master extends DBConnection {
	...
	//SAVE ASIGNATORY FUNCTION
	function save_asignatory(){
		extract($_POST);
		$data = "";
		foreach($_POST as $k =>$v){
			if(!in_array($k,array('id'))){
		...
		$check = $this->conn->query("SELECT * FROM `asignatory_list` where `name` = '{$name}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = " Asignatory already exist.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
			$sql = "INSERT INTO `asignatory_list` set {$data} ";
			$save = $this->conn->query($sql);
		}else{
			$sql = "UPDATE `asignatory_list` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if($save){
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"New Asignatory successfully saved.");
			else
				$this->settings->set_flashdata('success',"Asignatory successfully updated.");
		}else{
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error."[{$sql}]";
		}
	...
	//DELETE ASIGNATORY FUNCTION
	function delete_asignatory(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `asignatory_list` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Asignatory successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	//FUNCTION SAVE EMPLOYEE LEAVE TYPE
...
switch ($action) {
	...
	break;
	case 'save_asignatory':
		echo $Master->save_asignatory();
	break;
	case 'delete_asignatory':
		echo $Master->delete_asignatory();
	break;
