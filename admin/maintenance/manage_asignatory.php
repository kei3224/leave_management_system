...
if (isset($_GET['id']) && $_GET['id'] > 0) {
    ...
    $qry = $conn->query("SELECT * from `asignatory_list` where id = '{$_GET['id']}' ");
    if ($qry->num_rows > 0) {
...
<div class="container-fluid">
    <form action="" id="asignatory-form">
...
<script>
    $(document).ready(function() {
        $('#asignatory-form').submit(function(e) {
            e.preventDefault();
            var _this = $(this)
            var _this = $(this)
            $('.err-msg').remove();
            start_loader();
            $.ajax({
                url: _base_url_ + "classes/Master.php?f=save_asignatory",
